...
// API
import { getUserFavoriteBrands, checkUserHasFavoriteBrands } from '../services/api/interestbrandApi';
...
function HomePage() {
    ...
    // 관심 브랜드
    const [likedBrandsMirror, setLikedBrandsMirror] = useRecoilState(likedBrandsAtom);
    ...
    const refreshBenefits = async (favsFromServer = []) => {
        ...
        try {
            const groups = await fetchFavoriteBenefits();
    ...
    const isLoadingAll =
        recLoading || favLoading || benefitLoading || interestOrPaymentBenefits.length === 0;
    ...
    // 홈 섹션 조건
    const hasLikedBrands = favBrands.length > 0;
    ...
    const previewBenefits =
        serverFlatBenefits.length > 0 ? serverFlatBenefits : localFlatBenefits;
